...
PHOTOS_DB = "tests/Test-14.0.0.photoslibrary/database/photos.db"
PHOTOS_DB_PATH = "/Test-14.0.0.photoslibrary/database/Photos.sqlite"
PHOTOS_LIBRARY_PATH = "/Test-14.0.0.photoslibrary"
...
PERSONS_DICT = {
    ...
    "Suzy": 2,
    "Maria": 2,
    "_UNKNOWN_": 1,  # On earlier versions, the photo of the statue has a detected face (IMG_4547.jpg) but it's not detected in Photos 9 dev preview
}
...
def test_photos_version(photosdb):
    assert photosdb.photos_version == 9
...
def test_attributes(photosdb):
    ...
    assert p.path.endswith(
        "tests/Test-14.0.0.photoslibrary/originals/D/D79B8D77-BFFC-460B-9312-034F2877D35B.jpeg"
    )
...
def test_attributes_2(photosdb):
    ...
    assert p.path.endswith(
        "tests/Test-14.0.0.photoslibrary/originals/E/E9BC5C36-7CD1-40A1-A72B-8B8FAC227D51.jpeg"
    )
    assert not p.ismissing
...
def test_labels(photosdb):
    """test PhotoInfo.labels"""
    for uuid, labels in UUID_LABELS.items():
