...
@click.option(
    ...
    "To prevent files not generated by osxphotos from being deleted, you may specify one or more patterns "
    "in a file named `.osxphotos_keep` in the export directory. "
    "This file uses the same format as a .gitignore file and may contain comments (lines starting with #) "
    "Reference https://git-scm.com/docs/gitignore#_pattern_format for details. "
    "In addition to the standard .gitignore rules, the pattern may also be the absolute path to a file or directory. "
    ...
    "you can specify `**/*.txt`. "
    "If present, tje .osxphotos_keep file will be read after the export is completed and any patterns found in the file "
    "will be added to the list of patterns to keep. "
    "See also --keep.",
)
@click.option(
    "--keep",
    ...
    "Reference https://git-scm.com/docs/gitignore#_pattern_format for details. "
    "In addition to the standard .gitignore rules, KEEP_PATTERN may also be the absolute path to a file or directory. "
    "For example if export destination is `/Volumes/Photos` and you want to keep all `.txt` files, "
    ...
    "Patterns may also be included in a file named `.osxphotos_keep` in the export directory. "
    "If present, this file will be read after the export is completed and any patterns found in the file "
    "will be added to the list of patterns to keep. "
    "The file uses the same format as a .gitignore file and may contain comments (lines starting with #) "
    "and one pattern per line. ",
)
@click.option(
...
def collect_files_to_keep(
    ...
    matcher = osxphotos.gitignorefile.parse_pattern_list(keep_patterns, export_dir)
    keepers = []
